...
The frozen registration is [the thesis artifact](evidence/costco/frozen_thesis.json) plus its [registration manifest](evidence/costco/registration_manifest.json). A one-minute read-only diagnostic then returned `40025` for both protected Reality endpoints; the recorder correctly marked it `INCOMPLETE` in [the sanitized diagnostic](evidence/costco/diagnostic_20260924T052600Z.json). This is a gate check, not the event result.
