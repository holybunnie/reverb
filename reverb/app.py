...
def render_app_html(snapshot: PreviewSnapshot, *, risk_budget: str | None = None,
                    ...
                    morning_report_html: str | None = None) -> str:
...
.risk {{ color:var(--gold); font-weight:700; }} .muted {{ color:var(--muted); font-size:14px; }} .divider {{ height:1px; background:var(--line); margin:25px 0; }} .small {{ font-size:13px; color:var(--muted); }} a {{ color:var(--mint); }} code {{ color:var(--mint); }} input,button {{ width:100%; padding:10px; margin:6px 0 12px; border-radius:8px; border:1px solid var(--line); background:var(--bg); color:var(--text); }} button {{ background:var(--mint); color:#071014; font-weight:700; cursor:pointer; }}
...
<form method="get" action="/app" class="card" style="margin-top:12px"><label>Risk budget <input name="risk" inputmode="decimal" placeholder="e.g. 50" value="{html.escape(risk_budget or '')}"></label><br><label>Timezone <input name="timezone" placeholder="Africa/Lagos" value="{html.escape(timezone_name or '')}"></label><br><button type="submit">Save for this session</button></form>
...
<div class="divider"></div>
<section><div class="eyebrow">Morning report</div><h2>Nothing to hide</h2>{morning_report_html or '<p>There is no recorded trade or refusal for this account yet. Once an event is authorized, the report will show the pre-registration, outcome, and the arithmetic behind any refusal.</p>'}<p class="small">This page is driven by the same structured decisions used by the MCP tools. It does not expose raw market data.</p></section>
